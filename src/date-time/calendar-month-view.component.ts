...
export class OwlMonthViewComponent<T> implements OnInit, AfterContentInit, OnDestroy {
    ...
    private createDateCell( date: T, daysDiff: number ): CalendarCell {
        ...
        const weekNumber = this.getWeekNumber(new Date(this.dateTimeAdapter.getTime(date)));

        return new CalendarCell(dayValue, dateName, ariaLabel, enabled, out, cellClass, weekNumber);
    }
